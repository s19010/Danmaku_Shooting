...
class Bullet extends SpriteActor {
    constructor(x, y) {
        const sprite = new Sprite(assets.get('sprite'), new Rectangle(0, 16, 16, 16)); // （デフォルト）
        // const sprite = new Sprite(assets.get('sprite'), new Rectangle(16, 16, 16, 16)); // （スキンA)
        // const sprite = new Sprite(assets.get('sprite'), new Rectangle(0, 0, 16, 16)); // （スキンB)
...
class Enemy extends SpriteActor {
    constructor(x, y) {
        ...
        this._interval = 120; // 敵が弾を撃ち出す間隔（デフォルト）
        // this._interval = 10; // 〃（ハードモード）
        // this._interval = 240; // 〃（イージーモード）
        this._timeCount = 0;
        this._velocityX = 0.3; // 敵の左右への移動スピード
